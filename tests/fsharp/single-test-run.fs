...
let private singleTestRun' cfg testDir =
    ...
    let skipIfNotExists file = processor {
        if fileExists file |> Option.isNone
    ...
    let runFSC_HW () = processor {
        // if exist test-hw.* (
        do! match Directory.EnumerateFiles(testDir, "test-hw.*") |> Seq.tryPick fileExists with
            | None -> NUnitConf.skip (sprintf "file '%s' not found" "test-hw.*")
            | Some _ -> processor {
                // if exist test.ok (del /f /q test.ok)
                use testOkFile = createTestOkFile () 
                // %CLIX% .\test-hw.exe && (
